...
export class FactureComponent implements OnInit {
 factures:Facture[];
 //factures2:Facture[];
 factures2: Array<Facture> = [];
 facturesustilisateur: Array<Facture> = [];
 facturesclien: Array<Facture> = [];
 typeuser="";
 idactive="";
 idboss="";
  ...
  getfacture(){
    this.factures.forEach(element => {
      ...
      fact.datecreation=element.datecreation;
      fact.nembremessageenv=element.nembremessageenv;
  ...
  calculeculetotalemois(a:String,b:String){
    if(this.typeuser=="client"){
      ...
      var moisselectionne=element.datecreation.substring(5,7);
      var anneeselectionner=element.datecreation.substring(0,4);
